...
5. 各トピック: 「### トピックタイトル」の見出し + 「**要旨**」+ 「**詳細解説**」の構成で**メールに実在する全記事**を出力。
   ただし上記【絶対厳守】に従い、**「詳細解説」はメール本文に根拠がある場合のみ**書く（根拠が無ければ要旨だけ／見出し訳だけ）。
   入力に無いトピック・分析・数値を足してブロックを水増ししてはならない。件数はメールの実記事数に一致させる。
