...
■ 出力構成:
  ...
  - <短信2>：…
  ### （メール本文の各特集記事の見出しを、そのまま日本語訳して使う）
  本文をそのまま訳す（設問形式の小見出し・箇条書きも保つ）。特集記事はメールにある数だけ全部。
...
■ 絶対厳守:
  ...
  - **分割翻訳の続編チャンクでは front matter・タイトル・既出の ## / ### 見出しを再掲しない**（本文の続きだけ出力。CNBC/Axiosと同じ継続ルール）。
  - **このプロンプトの例示語（「特集記事1」「特集記事2」「Deals」「速報まとめ」等の説明）をそのまま見出しや本文に書かない**。使うのは**メールの実際の見出し・本文だけ**。
  - **入力チャンクに新しい記事が無ければ、テンプレを埋めず何も出力しない**（「デジタル化の進展」「環境への配慮」「企業戦略の変化」等の**架空記事を絶対に作らない**）。英語のセクションラベル（`A tough call` 等）を英語のまま再掲しない。
