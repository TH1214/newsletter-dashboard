...
❌ 悪い例（メール実記事を汎用テーマに丸めて、本文に無い分析を創作）:
    ## エグゼクティブサマリー
  ...
  「雇用市場／経済影響／市場動向」という汎用ラベルに丸め、本文に無い分析（リモートワーク等）を創作している。

✅ 正しい形（メールの実記事だけを、その見出しと本文で）:
    ## エグゼクティブサマリー   ← 行数はメールの実記事数に一致（3件に丸めない）
    | 1 | 雇用統計 | 金曜の雇用統計を前に市場が神経質。|   ← メール実記事の1行要約
    | 2 | …（メールに実在する記事だけ続ける）
    ### 金曜の雇用統計待ち   ← メールの実際の記事見出しを日本語訳
    **要旨** メール本文のその記事の記述を忠実に日本語化するだけ（分析・一般論・見通しを足さない）

【構造的な強制ルール（最重要・必ず守る）】
- エグゼクティブサマリー表の行数 と ### 見出しの数 は、**メールに実在する記事数に必ず一致**させる。
  3件に丸めたり、件数合わせで水増ししたりしない。
- 各 ### 見出しは、**メールの実際の記事見出し（またはリード文）の日本語訳**にする。
  「雇用市場」「経済影響」「市場動向」「経済動向」「トピック1/2/3」のような**汎用ラベルは禁止**。
- 「詳細解説」は、**メール本文の該当記事の文章を訳して並べるだけ**。分析・一般論・将来見通し・架空の引用を足さない。
  該当記事の記述が乏しければ **要旨だけ** にして、詳細解説は書かない（無理に埋めない）。
